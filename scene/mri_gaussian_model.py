...
from flame_model.mrihead import MRIHead
...
class MRIGaussianModel(GaussianModel):
    def __init__(
        self,
        sh_degree: int,
        mesh_path="flame_model/assets/flame/head_template_mesh.obj",
        ...
        self.n_expr = n_expr
        print("Loding Head for Gaussian model...")
        self.flame_model = MRIHead(
            n_shape,
            n_expr,
            mesh_path=mesh_path,
            # add_teeth=True,
    ...
    def load_meshes(self, train_meshes, test_meshes, tgt_train_meshes, tgt_test_meshes):
        if self.flame_param is None:
            ...
            if not self.disable_flame_static_offset:
                ...
                if static_offset.shape[0] != num_verts:
                    static_offset = torch.nn.functional.pad(
                        static_offset,
                        (0, 0, 0, num_verts -
                         meshes[0]["static_offset"].shape[1]),
                    )
            else:
            ...
            for i, mesh in pose_meshes.items():
                ...
                print(
                    f"Loaded mesh parameters for timestep {i} of {self.num_timesteps}:\n"
                    f"  {'translation'.ljust(12)}: {self.flame_param['translation'][i]}\n"
                    f"  {'rotation'.ljust(12)}: {self.flame_param['rotation'][i]}\n"
                )                # print(f"Loaded mesh parameters for timestep {i}: ")
