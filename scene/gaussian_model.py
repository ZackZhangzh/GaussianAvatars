...
class GaussianModel:
    ...
    def get_xyz(self):
        if self.binding is None:
            ...
            xyz = torch.bmm(
                self.face_orien_mat[self.binding], self._xyz[..., None]).squeeze(-1)
    ...
    def create_from_pcd(self, pcd: Optional[BasicPointCloud], spatial_lr_scale: float):
        ...
        if pcd == None:
            ...
            fused_color = torch.tensor(np.random.random(
                (num_pts, 3)) / 255.0).float().cuda()
        else:
        ...
        if self.binding is None:
            ...
            scales = torch.log(torch.ones(
                (self.get_xyz.shape[0], 3), device="cuda"))
    ...
    def save_ply(self, path):
        ...
        if self.binding is not None:
            attributes.append(self.binding.detach().cpu().numpy()[:, None])
        ...
        # additionally save a global version
        if self.binding is not None:
            xyz = self.get_xyz.detach().cpu().numpy()
            attributes = [xyz, normals, f_dc,
                          f_rest, opacities, scale, rotation]
            if self.binding is not None:
                attributes.append(self.binding.detach().cpu().numpy()[:, None])
            attributes = np.concatenate(attributes, axis=1)
            elements[:] = list(map(tuple, attributes))
            ...
            # PlyData([el]).write(base + "_global" + ext)
    ...
    def load_ply(self, path, **kwargs):
        ...
        # optional fields
        binding_names = [
            p.name for p in plydata.elements[0].properties if p.name.startswith("binding")]
        if len(binding_names) > 0:
            binding_names = sorted(
                binding_names, key=lambda x: int(x.split('_')[-1]))
            binding = np.zeros(
                (xyz.shape[0], len(binding_names)), dtype=np.int32)
            for idx, attr_name in enumerate(binding_names):
                binding[:, idx] = np.asarray(plydata.elements[0][attr_name])
            self.binding = torch.tensor(
                binding, dtype=torch.int32, device="cuda").squeeze(-1)
    ...
    def prune_points(self, mask):
        if self.binding is not None:
            ...
            counter_prune.scatter_add_(0, binding_to_prune, torch.ones_like(
                binding_to_prune, dtype=torch.int32, device="cuda"))
        ...
        if self.binding is not None:
            ...
            self.binding_counter.scatter_add_(
                0, self.binding[mask], -torch.ones_like(self.binding[mask], dtype=torch.int32, device="cuda"))
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        if self.binding is not None:
            ...
            self.binding_counter.scatter_add_(0, new_binding, torch.ones_like(
                new_binding, dtype=torch.int32, device="cuda"))
